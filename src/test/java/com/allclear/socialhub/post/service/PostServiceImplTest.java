...
class PostServiceImplTest {
	...
	void createPostWithNonExistType() {
		...
		// when & then
		assertThatThrownBy(() -> postService.createPost(user.getId(), request)).isInstanceOf(
	...
	void updatePostWithoutTitle() {
		...
		// when & then
		assertThatThrownBy(() -> postService.updatePost(user.getId(), post.getId(), updateRequest)).isInstanceOf(
				DataIntegrityViolationException.class);
